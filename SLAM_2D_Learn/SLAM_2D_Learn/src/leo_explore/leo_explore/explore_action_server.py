...
class ExploreActionServer(Node):
    ...
    def __init__(self):
        ...
        # ------------------------------------------------------------
        # Parameters: same defaults as the working frontier_explorer node
        # ------------------------------------------------------------
    ...
    def deactivate_and_stop(self):
        with self.state_lock:
            self.exploring_active = False
            self.current_state = self.STATE_CANCELING
    ...
    def cancel_current_nav_goal(self):
        ...
        try:
            ...
            cancel_future.add_done_callback(
                lambda _future: self.get_logger().info('Cancel request sent to NavigateToPose goal')
            )
        except Exception as e:
