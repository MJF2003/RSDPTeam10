...
## ROS node for Yolov5 without msg (in ROS_node file)
### Main pose topics
...
## ROS node for Yolov5 without msg (in src file)
### Terminal A: 
source /opt/ros/jazzy/setup.bash 
export ROS_DOMAIN_ID=10 
ros2 launch realsense2_camera rs_launch.py align_depth.enable:=true enable_sync:=true
...
### Terminal B: 
source /home/team10/Dev/venv/bin/activate 
source ~/cv_ws/install/setup.bash 
export ROS_DOMAIN_ID=10

python ~/cv_ws/src/rsdp_perception/rsdp_perception/perception_typed_node.py \
--ros-args \
-p weights:=/home/team10/yolov5/train/blocks_bins_platform4/weights/best.pt \
-p imgsz:=416 \
-p conf:=0.25 \
-p block_attrs_dir:=/home/team10/yolov5/bins_blocks_dataset/block_attrs/clf_out \
-p bin_color_dir:=/home/team10/yolov5/bins_blocks_dataset/bin_color/clf_out \
-p min_attr_conf:=0.20 \
-p vote_window:=10 \
-p min_votes_to_output:=1 \
-p color_topic:=/camera/camera/color/image_raw \
...
source ~/cv_ws/install/setup.bash 
export ROS_DOMAIN_ID=10 
python3 ~/cv_ws/src/rsdp_perception/rsdp_perception/tracks_monitor_typed_node.py
