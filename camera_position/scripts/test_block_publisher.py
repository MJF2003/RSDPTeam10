...
class TestBlockPublisher(Node):
    ...
    def publish_test_blocks(self):
        ...
        # Test Block 1 - At the centre of the image, depth 1.5 metres
        block1 = DetectedBlock()
        block1.block_id = f"block_center_{self.counter}"
        block1.bbox = [646.91, 371.37, 50.0, 50.0]  # 图像中心位置 Centre of the image
        block1.depth = 1.0  # 1.0m away
        block1.confidence = 0.95
        
        # 创建测试方块2 - 在图像左侧，深度1.5米
        # Test Block 2 - On the left side of the image, depth 1.5 metres
        block2 = DetectedBlock()
        block2.block_id = f"block_corner_{self.counter}"
        block2.bbox = [400.0, 371.37, 50.0, 50.0]  # 左侧位置 Left-hand position
        block2.depth = 1.5  # 1.5m away
